...
def test_inject(tracer):
    ...
    with tracer.trace("global_root_span") as span:
        ...
        assert "_dd.p.dm=value" in headers[_HTTP_HEADER_TAGS]
        assert "_dd.p.other=value" in headers[_HTTP_HEADER_TAGS]
...
def test_inject_tags_large(tracer):
    ...
    # DEV: Limit is 128 for key=value
    meta = {"_dd.p.dm": ("x" * 128)[len("_dd.p.dm") - 1 :]}
    ctx = Context(trace_id=1234, sampling_priority=2, dd_origin="synthetics", meta=meta)
    tracer.context_provider.activate(ctx)
    with tracer.trace("global_root_span") as span:
        headers = {}
